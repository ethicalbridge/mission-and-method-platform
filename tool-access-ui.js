...
const setState=granted=>{
 document.querySelectorAll('[data-tool-access-link]').forEach(link=>{if(!link.dataset.preserveLabel)link.textContent=granted?`Open ${link.dataset.toolName} →`:'Unlock with access code →';if(granted){link.href=link.dataset.toolDestination;link.target='_blank';link.rel='noopener';}else{link.href='#tool-access';link.removeAttribute('target');}});
